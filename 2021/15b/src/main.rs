...
fn solve(input: Input) -> u32 {
    let tile = input.parse::<Grid<u32>>();
    let theight = tile.height();
    let twidth = tile.width();
    let bounds = GridBounds::new(tile.height() * 5, tile.width() * 5);
    let grid = Grid::<u32>::from_fn(bounds, |(y, x)| {
        let tx = u32::try_from(x / twidth).unwrap();
        let x = x % twidth;
        let ty = u32::try_from(y / theight).unwrap();
        let y = y % theight;
        let risk = (tile[(y, x)] + tx + ty) % 9;
